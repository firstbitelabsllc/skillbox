...
try {
  ...
  await page.screenshot({ path: path.join(output, "skillbox-demo.png") });
  await page.waitForTimeout(4000);
  const video = page.video();
  await context.close();
  await video.saveAs(path.join(output, "skillbox-demo.webm"));
  await video.delete();
  for (const name of ["skillbox-demo.png", "skillbox-demo.webm"])
  ...
  execFileSync(
    ...
    [
      "-y",
      "-i",
      path.join(output, "skillbox-demo.webm"),
      "-an",
      "-c:v",
      "libx264",
      "-pix_fmt",
      ...
      "-movflags",
      "+faststart",
      path.join(output, "skillbox-demo.mp4"),
    ],
    { stdio: "ignore" },
  );
